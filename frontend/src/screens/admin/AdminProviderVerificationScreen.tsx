...
type ProviderVerifNavProp = NativeStackNavigationProp<
  AdminStackParamList,
  'AdminProviderVerification'
>;
type ProviderVerifRouteProp = RouteProp<AdminStackParamList, 'AdminProviderVerification'>;

interface Props {
  navigation: ProviderVerifNavProp;
  route: ProviderVerifRouteProp;
}

export const AdminProviderVerificationScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const [items, setItems] = useState<AdminVerificationItem[]>(adminService.getProviderVerifications());

  useEffect(() => {
    const unsubscribe = adminService.subscribe(() => {
      setItems(adminService.getProviderVerifications());
    });
    return unsubscribe;
  }, []);
  ...
  const handleUpdateStatus = (item: AdminVerificationItem, newStatus: VerificationStatus) => {
    Alert.alert(
      `Mark as ${newStatus}`,
      `Update verification status for provider ${item.name} to ${newStatus}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            // Also sync the provider record
            adminService.updateProviderVerification(item.targetId, newStatus);
          },
        },
      ]
    );
  ...
  return (
    <ScreenContainer
      ...
      header={
        <Header
          ...
          subtitle="Commercial documents & host accreditation"
          showBack
    ...
    >
      <View style={[styles.content, { padding: spacing.md }]}>
        {/* Status Tabs: Pending, Verified, Rejected, Suspended, All */}
        ...
        {filteredItems.length === 0 ? (
          <Card variant="flat" padding="large" style={styles.emptyCard}>
            ...
            >
              There are no provider verification items with status: {selectedStatus}.
            </Text>
          </Card>
        ) : (
